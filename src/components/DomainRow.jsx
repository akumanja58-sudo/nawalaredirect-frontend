...
import ISPBadges from './ISPBadges';
import api from '../api';
...
export default function DomainRow({ domain, onRefresh, groups = [] }) {
  ...
  async function handleCheckISP() {
    setCheckingISP(true);
    try { await api.post(`/api/domains/${domain.id}/check-isp`); await onRefresh(); }
    catch (e) { alert('ISP check gagal: ' + (e.response?.data?.error || e.message)); }
    ...
    <div style={S.row}>
      ...
      <div style={S.info}>
        <div style={S.urlRow}>
          ...
          {domain.group_name && (
            <span style={S.groupTag}>{domain.group_name}</span>
          )}
        ...
        <ISPBadges ispStatus={domain.isp_status} />
        ...
        {editingGroup && (
          <div style={S.editRow}>
            ...
            <input style={S.editInput} list="group-list" value={groupInput}
              ...
              placeholder="nama group (cth: slot)" autoFocus
      ...
      <div style={S.actions}>
        ...
        <button style={{ ...S.actionBtn, color: 'var(--blue)' }} onClick={handleCheckISP} disabled={checkingISP} title="Cek ISP">
          {checkingISP ? '...' : '🇮🇩'}
        </button>
        <button style={S.actionBtn} onClick={handleCheck} disabled={checking} title="Refresh check">
          {checking ? '...' : '↻'}
        </button>
...
const S = {
  row: { display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg2)' },
  ...
  url: { fontSize: 13, fontWeight: 500, color: 'var(--text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 360, fontFamily: 'var(--mono)' },
  groupTag: { fontSize: 11, fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: 'rgba(99,102,241,0.1)', color: '#6366f1', border: '1px solid rgba(99,102,241,0.2)', letterSpacing: '0.3px' },
